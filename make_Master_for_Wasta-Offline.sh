...
#      $InternetURLPrefix which is http://. If mirror.list can't be generated a
#      warning is given and the script aborts.
...
echo -e "\nChecking for the presence of the $SYNCWASTAOFFLINESCRIPT script..."
sleep 2s
...
case $# in
    0) 
      ...
      if [[ "$COPYTODIR" == "$COPYFROMDIR" && "x$COPYTODIR" != "x" && "x$COPYFROMDIR" != "x" ]]; then
        echo "The computer's mirror.list file points to a base_path for its mirror:"
        echo "   $COPYTODIR"
...
echo "  Destination mirror (parameter 1): $COPYTODIR"
